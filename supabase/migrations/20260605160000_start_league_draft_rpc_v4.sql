-- v4: fix empty player_ids array bug (array_length returns NULL for empty arrays)
...
BEGIN
  ...
  -- 5. Build categories + settings JSON
  v_categories := COALESCE(v_entry.categories, ARRAY[]::TEXT[]);
  v_settings   := COALESCE(v_entry.draft_settings, '{}'::JSONB);
  ...
  IF v_entry.is_multiplayer THEN
    ...
    INSERT INTO public.drafts (
      ...
      is_multiplayer, status, draft_settings, created_at
    )
    VALUES (
      auth.uid(),
