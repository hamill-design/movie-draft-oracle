-- ============================================================
-- start_league_scheduled_draft v2
--
-- Respects the new player_ids column: when non-empty, only
-- those league members are used as draft participants.
-- When empty (default), all current members are included.
-- ============================================================
...
BEGIN
  ...
  IF v_entry.draft_id IS NOT NULL THEN
    -- Draft already started — return existing draft id
  ...
  IF v_entry.is_multiplayer THEN
    ...
    -- Add the admin as host participant
