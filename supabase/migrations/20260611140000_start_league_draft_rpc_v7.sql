-- v7: fix 'record "v_entry" has no field "title"'.
--
-- Same family of bug as v5/v6: league_drafts has never had a "title"
-- column (confirmed against live information_schema — its columns are
-- id, league_id, draft_id, added_at, season_id, scheduled_at, draft_type,
-- notes, theme, categories, is_multiplayer, player_ids). Referencing
-- v_entry.title threw immediately — COALESCE can't rescue a reference to a
-- field that doesn't exist on the record, so the 'League Draft' fallback
-- was never actually reachable.
--
-- Replaced with COALESCE(NULLIF(v_entry.notes, ''), 'League Draft'):
-- "notes" is a real, nullable text column on league_drafts (the optional
-- note an admin can add when scheduling), so if they wrote one it's used
-- as the draft's title; otherwise it falls back to 'League Draft' as
-- originally intended.
--
-- Audited every other v_entry.<field> reference in this function against
-- the live league_drafts columns above — league_id, draft_id, draft_type,
-- theme, categories, is_multiplayer, and player_ids are all real columns,
-- so "title" was the only remaining bad reference.
...
BEGIN
  ...
  IF v_entry.is_multiplayer THEN
    ...
    VALUES (
      auth.uid(),
      COALESCE(NULLIF(v_entry.notes, ''), 'League Draft'),
      v_entry.draft_type,
