-- Update scoring to use fixed 20/80 weights (Box Office/Critical) instead of consensus-based weight adjustment
-- This migration updates the calculate_new_movie_score function to use fixed weights

-- Update the function with fixed 20/80 weights
...
-- Recalculate all existing draft_picks with the fixed 20/80 weights
...
BEGIN
  ...
  RAISE NOTICE 'Recalculated scores for % draft picks with fixed 20/80 weights', updated_count;
