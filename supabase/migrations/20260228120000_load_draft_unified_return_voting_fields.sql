-- Return voting_ends_at and allow_public_voting from load_draft_unified so all participants
-- (including non-host) get voting state and can enable "View Final Scores" when appropriate.
-- Must DROP first because return type (OUT parameters) changed.
DROP FUNCTION IF EXISTS public.load_draft_unified(uuid, uuid);

CREATE FUNCTION public.load_draft_unified(p_draft_id uuid, p_participant_id uuid)
 RETURNS TABLE(
   draft_id uuid,
   draft_user_id uuid,
