-- Implement consensus scoring system with three-layer consensus and Box Office-Audience weight adjustment
-- This migration updates the calculate_new_movie_score function with the new consensus scoring algorithm

-- Update the function with consensus scoring
...
BEGIN
  ...
  IF box_office_score > 0 AND audience_raw_avg > 0 THEN
    ...
    -- Adjust weights based on consensus (base 20% Box Office, 80% Critical)
...
-- Recalculate all existing draft_picks with the new consensus scoring system
...
WHERE calculated_score IS NOT NULL
  AND (
    ...
    OR letterboxd_rating IS NOT NULL
  );
...
BEGIN
  ...
  FROM public.draft_picks
  WHERE imdb_rating IS NOT NULL OR letterboxd_rating IS NOT NULL;
  
  RAISE NOTICE 'Recalculated scores for % draft picks with consensus scoring system', updated_count;
  RAISE NOTICE '% movies with Box Office data', with_box_office;
  RAISE NOTICE '% movies with Critics data (RT/Metacritic)', with_critics;
  RAISE NOTICE '% movies with Audience data (IMDB/Letterboxd)', with_audience;
END $$;
