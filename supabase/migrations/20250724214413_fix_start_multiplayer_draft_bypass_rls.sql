-- Fix the start_multiplayer_draft function to bypass RLS for initial draft lookup
...
BEGIN
  ...
  -- BYPASS RLS: Get the draft directly without relying on RLS policies
  -- This avoids the session variable scope issues within SECURITY DEFINER functions
  ...
  -- Re-enable RLS for subsequent queries
  PERFORM set_config('row_security', 'on', true);

  IF NOT FOUND THEN
    RAISE LOG 'Draft not found with ID: %', p_draft_id;
    RAISE EXCEPTION 'Draft not found';
  END IF;

  RAISE LOG 'Found draft: ID=%, user_id=%, guest_session_id=%, is_multiplayer=%', 
    v_draft_record.id, v_draft_record.user_id, v_draft_record.guest_session_id, v_draft_record.is_multiplayer;

  -- EXPLICIT PERMISSION CHECKS (bypassing RLS)
  -- 1. Owner access (authenticated user)
  IF auth.uid() IS NOT NULL AND v_draft_record.user_id = auth.uid() THEN
    v_has_access := true;
    RAISE LOG 'Access granted: authenticated owner';
  END IF;
  ...
  IF p_guest_session_id IS NOT NULL AND v_draft_record.guest_session_id = p_guest_session_id THEN
    v_has_access := true;
    RAISE LOG 'Access granted: guest owner';
  END IF;
  
  -- 3. Host participant access (also bypass RLS for this check)
  PERFORM set_config('row_security', 'off', true);
  IF EXISTS (
    ...
    v_has_access := true;
    RAISE LOG 'Access granted: host participant';
  END IF;
  PERFORM set_config('row_security', 'on', true);

  IF NOT v_has_access THEN
    RAISE LOG 'Access denied for user_id=% guest_session_id=% on draft_id=%', 
      auth.uid(), p_guest_session_id, p_draft_id;
  ...
  -- Get participants ordered by join time (bypass RLS for this too)
  PERFORM set_config('row_security', 'off', true);
  ...
  IF v_participants_list IS NULL OR array_length(v_participants_list, 1) < 2 THEN
    RAISE LOG 'Insufficient participants: count=%', COALESCE(array_length(v_participants_list, 1), 0);
    RAISE EXCEPTION 'Need at least 2 participants to start draft';
  END IF;

  RAISE LOG 'Starting draft with % participants', array_length(v_participants_list, 1);
  ...
  RAISE LOG 'Draft setup: categories=%, participants=%, total_picks=%', 
    v_categories_count, v_participant_count, v_total_picks;
  ...
  RAISE LOG 'Generated turn order with first turn user: %', v_first_turn_user_id;
